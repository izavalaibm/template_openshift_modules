...
resource "null_resource" "ign_config_generated" {
  depends_on = [
    null_resource.generate_ign_config,
    camc_scriptpackage.get_interfaces,
    camc_scriptpackage.get_cluster_key,
    camc_scriptpackage.get_bootstrap_sec_ign,
    camc_scriptpackage.get_worker_ign,
    camc_scriptpackage.get_master_ign,
    null_resource.generate_ign_config_dependsOn,
  ]
  provisioner "local-exec" {
